...
FROM product;
-- product_qty_type NULL is also displayed as bulk.
...
SELECT vendor_id, COUNT(*) AS vendor_booth_assignments_per_vendor_id FROM vendor_booth_assignments 
GROUP BY vendor_id;
...
SELECT c.customer_id, c.customer_first_name, c.customer_last_name, SUM(cp.quantity*cp.cost_to_customer_per_qty) AS total_spend FROM customer c 
INNER JOIN customer_purchases cp 
	ON c.customer_id = cp.customer_id
GROUP BY c.customer_id HAVING SUM(cp.quantity*cp.cost_to_customer_per_qty) > 2000
