...
```
Type 1 SCD overwrites old data with new data, thereby losing history.

Type 2 SCD on the other hand, adds a new record for each change, preserving history.

Type 1 is simpler and suitable when history is not needed, whereas Type 2 is necessary for tracking changes and trend analysis.
```
