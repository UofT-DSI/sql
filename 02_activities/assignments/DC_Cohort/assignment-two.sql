...
SELECT 
product_name,
SUBSTR(product_name, INSTR(product_name, '-') + 1) AS raw_description
FROM product
LIMIT 10;

SELECT 
product_name,
TRIM(SUBSTR(product_name, INSTR(product_name, '-') + 1)) AS cleaned_description
FROM product
LIMIT 10;
...
SELECT 
    v.vendor_name,
    p.product_name,
    ( (SELECT COUNT(*) FROM customer) * 5 * vi.original_price ) AS total_revenue
