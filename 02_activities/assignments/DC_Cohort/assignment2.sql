...
vendor_products_per_customer AS (
    SELECT
        vp.vendor_name,
        vp.product_name,
        (vp.original_price * 5) AS revenue_per_customer
    FROM vendor_products vp
    CROSS JOIN customer c   -- one row per (vendor, product, customer)
)
SELECT
    vendor_name,
    product_name,
    SUM(revenue_per_customer) AS total_revenue
FROM vendor_products_per_customer
...
SET current_quantity = COALESCE((
    ...
    ORDER BY vi.market_date DESC      -- use market_date, not inventory_date
    LIMIT 1
), 0);