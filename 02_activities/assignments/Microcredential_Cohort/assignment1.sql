...
SELECT c.customer_last_name, c.customer_first_name, SUM(c_p.quantity*c_p.cost_per_quantity) as spending

FROM customer_purchases c_p
...
SELECT customer_id, market_date , strftime('%m', market_date) as Month, strftime('%Y', market_date) as Year,  SUM(quantity*cost_per_quantity) as money_spent 
FROM customer_purchases
