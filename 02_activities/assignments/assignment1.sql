...
HINT: This query requires you to join two tables, use an aggregate function, and use the HAVING keyword. */
	...
	SELECT
		...
		ROUND(SUM(cp.quantity * cp.cost_to_customer_per_qty),2) total_spent
	FROM
		customer c
	INNER JOIN
...
and year are! */
	...
	FROM customer_purchases
	ORDER BY customer_id, month, year;
...
but remember, STRFTIME returns a STRING for your WHERE statement!! */
	...
	SELECT 
		...
		CASE STRFTIME('%m', market_date)
			...
			ELSE '' END month,
		SUM(quantity * cost_to_customer_per_qty) total_amount_spent
	FROM customer_purchases
	WHERE year = '2022'
	AND month = 'April'
