...
SELECT vba.vendor_id,
	COUNT(vendor_id) AS tot_rentals
...
SELECT cp.customer_id, customer_first_name, customer_last_name, ROUND(SUM(cp.quantity*cp.cost_to_customer_per_qty),2) AS tot_spent
FROM customer_purchases AS cp
	INNER JOIN customer AS c
		ON cp.customer_id = c.customer_id
GROUP BY cp.customer_id
HAVING tot_spent>2000
ORDER BY customer_last_name ASC, customer_first_name ASC;
