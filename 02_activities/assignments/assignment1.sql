...
case
        when product_name like '%pepper%' or product_name like '%PEPPER%' then 1
		else 0
end as pepper_flag
from product;
...
select DISTINCT vendor_id, count(booth_number) as 'vendor_booth_count' from vendor_booth_assignments group by vendor_id;
...
(
 select customer_id, quantity, cost_to_customer_per_qty, strftime('%m',market_date) as month, strftime('%Y',market_date) as year from customer_purchases
)
select customer_id, sum(quantity*cost_to_customer_per_qty) as total_customer_spent
from cust_purchases
where cast(month as int) = 4 and cast(year as int) = 2022
