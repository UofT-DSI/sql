...
SELECT v.vendor_name, v.vendor_id, count(booth_number) as [count] FROM
vendor as v 
INNER JOIN 
vendor_booth_assignments as vba
ON v.vendor_id = vba.vendor_id
...
select c.customer_last_name, c.customer_first_name, SUM(cp.quantity * cp.cost_to_customer_per_qty) AS total_spent FROM
customer c 
INNER JOIN
