...
SELECT customer_id, market_date
FROM 
(SELECT customer_id, market_date,ROW_NUMBER() OVER (PARTITION BY customer_id ORDER BY market_date DESC) AS visit_number FROM customer_purchases) AS numbered_visits
...
SELECT market_date, total_sales, 'Best Day' AS sales_rank
FROM ranked_sales
WHERE best_rank = 1

UNION

SELECT market_date, total_sales, 'Worst Day' AS sales_rank
FROM ranked_sales
WHERE worst_rank = 1
...
(product_id INT, product_name TEXT, product_size TEXT, product_qty_type TEXT, snapshot_timestamp TEXT);
...
INSERT INTO product_units (product_id, product_name, product_size, product_qty_type, snapshot_timestamp)
VALUES (17, 'Blueberry Pie', '10"', 'unit', CURRENT_TIMESTAMP); 
