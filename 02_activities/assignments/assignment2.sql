...
--Unique market dates per customer  
SELECT 
  customer_id,
  market_date,
...
--write another query that uses this one as a subquery
...
-- Step 1: Sales grouped by market date
...
-- Step 2: Rank the dates by sales (highest and lowest)
ranked_sales AS (
  SELECT 
    market_date,
    total_sales,
...
-- Step 3: Union best day and worst day
SELECT 
  market_date,
  total_sales,
...
SELECT 
  vp.vendor_name,
  vp.product_name,
  SUM(5 * vp.product_price) AS total_earned
...
VALUES (
  999,                          -- choose an ID not already taken
  'Apple Pie',                  -- product_name
  '1 slice',                    -- product_size
  'unit',                       -- product_qty_type (important!)
  3.99,                         -- product_price
...
-- Determine how to get the "last" quantity per product
SELECT vi.quantity
FROM vendor_inventory vi
WHERE vi.product_id = ?  -- Assuming we need to link via product_id
