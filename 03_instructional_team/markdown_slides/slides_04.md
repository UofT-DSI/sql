...
- The usage? Maybe comparing a set of stored values to ensure the input matches what is expected. Some characters may look the same, but would have different `UNICODE` values. 
  - e.g. `UNICODE('0')` will result in '48' whereas `UNICODE('O')` will result in '79'
