...
quiz(
  ...
  ,random_answer_order = TRUE
    ...
    question("Suppose you wanted to classify any 'rock','alternative',and 'metal' genres into a single category: 'favourites', but want to classify 'Rock & Roll' and 'Blues into a separate category: 'oldies'. What is the correct case statements?",
  answer("```
...
```")
  ,allow_retry = TRUE
  ,correct = "Correct! While you could write the case statement without a like statement and achieve a similar result, there may be more complex scenarios where this is harder. In those instances, the order of the case statement is crucial, as once a row is evaluated that it meets a criteria, it won't be re-evaluated."
  )
)
```
...
quiz(
  ...
  question("Which of the following queries correctly joins tables to output all tracks and their corresponding arists and albums?",
    answer("```
