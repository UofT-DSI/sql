...
select vendor_id,count( booth_number|market_date) from vendor_booth_assignments 
GROUP by vendor_id;
...
select  c.customer_last_name, c.customer_first_name, SUM(quantity*cost_to_customer_per_qty) as purchase  from customer_purchases cp
...
select customer_id, STRFTIME('%m',market_date) as month, STRFTIME('%Y',market_date) as year ,SUM(quantity * cost_to_customer_per_qty) as money from customer_purchases
