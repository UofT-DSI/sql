...
SELECT 
	*, 
	COUNT(*) OVER (PARTITION BY customer_id, product_id) AS number_of_product_id_purchased FROM customer_purchases 
...
SELECT 
    market_date,
    sales,
    RANK() OVER (ORDER BY sales DESC) AS best_total_sales,
    RANK() OVER (ORDER BY sales ASC) AS worst_total_sales
FROM 
    sales_data;
