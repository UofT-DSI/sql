...
- Reluctant Data Scientist (*do we even need this model?*) 
  - Currently doing Data Governance and Ethics implementation at Thomson Reuters
- This is Alto `r structure("\U1F449", class = "emoji")`
]
...
- Data Modelling
- Data Structures, Schemas, Basic Data Management, Normal Forms
- Basic SQL Syntax
- Advanced SQL Syntax
...
- Industry Case Study with Zia Barbar
...
- Our database for living coding
- All in class code
...
### Homework (ungraded, but worth doing)
- At the end of each SQL module, I will provide you with 3-10 queries to write
  - Covers the topics of the modules
- Review answers in Tutorials with Salaar
...
### Grading (specific rubrics tbd)
- Three Assignments
  1. Data Model Design, due at the end of week 2, start in week 1: 40%
  2. SQL Skills, due at the end of week 3, start in week 2: 40%
  3. Class Participation (reflection), due at the end of week 3, work on any time: 10%

- Class Attendance: 10%
  - Let Salaar or myself know if you are unable to attend a lesson
...
- Broad observations about Open Source:
...
- Broad observations about enterprise systems:
  ...
  - Tend to "lock in" business/organizations
      - Migration is costly, sometimes outrageously so
  - Newer players (Snowflake, Databricks, etc) and cloud providers (Azure, AWS, Google, etc) offer a lot more functionality than just database querying
...
- Relational databases are a collection of tables, views, procedural code, and other SQL-assisting artefacts
  - Generally the data stored in a database will related to a real-world concept
  ...
  - Tables are normalized (we'll get into Normalization Forms later in the course)
...
- Data Marts are created from Data Warehouses to focus on a single subject 
  ...
  - Some denormalized tables may be normalized or even undergo even greater normalization
...
- Data Lakes allow on-demand access to raw, semi-structured, structured, and unstructured data
  ...
  - Commonly data sources for machine learning pipelines live in data lakes
...
- Data types are important:
  - They affect operation speed, storage size, data validity
      - Speed: it's much computationally less expensive to compute smaller values
      - Storage: small is usually better, but the wrong size will affect systems, for example Psy's Gangnam Style exceed 2,147,483,647 (32-bit signed, 2^32/2-1) views, causing YouTube to expand the view counter to 64-bit (~9.2b if signed)
...
### Entity Relationship Diagrams
- Entity Relationship Diagrams (ERDs) are diagrams depicting the structure of tables within database
...
**One-to-Many**: where multiple rows within a table can be referenced by multiple rows in another table

Table 1: Employee — Table 2: Employee Type

For this example, imagine the following:

Previously, A was an instructor at DSI (python) _and_ a TA at DSI (SQL). Thomas remains the instructor for SQL. Salaar is now the TA. If a table contained Employee ID and Employee Type ID, A would be listed twice.
...
We'd create additional Many-to-Many relationships if we created a course table, because A and Salaar are involved in multiple courses!
...
- Our Farmers Market database: 
  ...
  - Relationships between these tables
      ...
      - product and customer_purchases: *what product a customer has bought*
      - product and vendor_inventory: *what products each vendor has on available*
...
- Our (partial) Farmers Market database: 
  - product shares a One-to-Many relationship with vendor_inventory on product_id
  - product_category shares a One-to-Many relationship with product on product_category_id
...
- Our (partial) Farmers Market database: 
  - vendor_id is the PK for vendor, which shares a One-to-Many relationship with vendor_inventory on vendor_id (FK)
  - product_id is a FK for vendor_inventory (*so elsewhere in this diagram, we'd connect this to a PK of another table*)
  - market_date is the PK for vendor_inventory
      - **why?**
...
**Q1)**
Create an example logical model for a small bookstore. `r structure("\U1F4DA", class = "emoji")`

At the minimum it should have employee, order, sale, customer, and book entities. Determine sensible column and table design based on what you know about these concepts. Keep it simple, but work out sensible relationships to keep tables reasonably sized. Include a date table. There are several tools online you can do this, I'd recommend [Lucidchart](https://www.lucidchart.com/pages/).

**Q2)**
We want to create employee shifts, splitting up the day into morning and evening. Add this to the ERD.

**Q3)**
The store wants to keep customer addresses. Propose two architectures for the CUSTOMERS_ADDRESS table, one that will retain changes, and another that will overwrite. Which is type 1, which is type 2? *Hint, search type 1 vs type 2 slowly changing dimensions*

**Q4)**
Review the AdventureWorks ERD here: [link](https://i.stack.imgur.com/LMu4W.gif)
...
```{r echo=FALSE}
#library(renderthis)
#renderthis::to_pdf("~/Documents/GitHub/02-intro_sql/Slides/slides_01.Rmd")
```