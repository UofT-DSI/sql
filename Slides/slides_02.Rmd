...
- The "OUTER" in `RIGHT OUTER JOIN` is optional
  - Generally, OUTER seems to be excluded but either are correct
- `RIGHT JOIN` are somewhat frowned upon, but sometimes they make sense
...
- Annoyingly, SQLite does not support `FULL OUTER JOIN` (_it really should_), but there is a work around to produce the results
  - We'll do it in Module 4
...
- Be weary of combining `MIN` & `MAX` with other aggregating functions like `SUM` or `AVG`
