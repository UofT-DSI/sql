...
# CONFIGURATION
folder_md="./test_slides"
folder_output="./pdf" # This will be used for both PDF and HTML
...
for markdown_file in $markdown_files; do
    ...
    if [ "$output_type" = "--html" ]; then
        ...
        marp "$markdown_file" --output "$output_file" --html --allow-local-files ${theme_path:+--theme $theme_path} # &> /dev/null
    elif [ "$output_type" = "--pdf" ]; then
        # Generate PDF
        output_file+=".pdf"
        echo "  - Generating PDF: $output_file"
        marp "$markdown_file" --output "$output_file" --pdf --allow-local-files --pdf-notes ${theme_path:+--theme $theme_path} # &> /dev/null
    else
        # nahhh
        show_help
        exit 1
