...
--Write a query that returns all customer purchases and a new calculated column 'price' (quantity * cost_to_customer_per_qty), filtered by customer IDs between 8 and 10 (inclusive) using either:
...
SELECT vendor_id, COUNT(*) AS rentals_count  
FROM vendor_booth_assignments 
GROUP BY vendor_id
ORDER BY rentals_count DESC;

--The Farmer’s Market Customer Appreciation Committee wants to give a bumper sticker to everyone who has ever spent more than $2000 at the market. 
...
SELECT c.customer_id, c.customer_first_name, c.customer_last_name, SUM(quantity * cost_to_customer_per_qty) AS total_spent
...
--Temp Table
--Insert the original vendor table into a temp.new_vendor and then add a 10th vendor: Thomass Superfood Store, a Fresh Focused store, owned by Thomas Rosenthal
...
FROM customer_purchases
WHERE strftime('%m', market_date) = '07' AND strftime('%Y', market_date) = '2022'
