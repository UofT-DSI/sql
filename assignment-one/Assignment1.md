...
***
Market_date_info and vendor_booth_assignments are linked through the market_date column. This is a one to many relationship where one record in market_date_info represents one market date. The same market date can appear in many rows in vendor_booth_assignments (multiple vendors can be assigned booths on the same date). So it is a one to many relationships.
